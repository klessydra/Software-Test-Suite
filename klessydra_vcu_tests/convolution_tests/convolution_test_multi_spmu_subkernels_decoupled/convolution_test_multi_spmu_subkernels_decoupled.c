	...
	array_boundaries[2] = A_ORDER;
		...
		sync_barrier_thread_registration();
			...
			if (Klessydra_get_coreID() == 0) {
				printf("\nMultithread test of %dx%d matrix:\t N of loops:%d\n",A_ORDER,A_ORDER,loop_index);
				printf("Replication is ON\n",A_ORDER,A_ORDER);
				// ENABLE COUNTING -------------------------------------------------------------------------
				final_perf_ld=0;
				__asm__("csrrw zero, mcycle, zero;"
		...
		if (Klessydra_get_coreID() == 0) {
			...
			kmemstr((void*)((int*)output_compare_s0 				),
		...
		if (Klessydra_get_coreID() == 1) {
			...
			kmemstr((void*)((int*)output_compare_s1 				),
			 				(void*)((int*)spmaddrC+memory_offset[off_idx]	),
							SIZE_OF_INT*(	A_ORDER*A_ORDER));
			// DISABLE COUNTING AND SAVE MCYCLE -------------------------------------------------------
			#ifdef PRINT_NUM_CYCLES
		...
		if (Klessydra_get_coreID() == 0) {
			#ifdef PRINT_NUM_CYCLES
				// printf("Replicatiion is %d (0=off, 1=on) \n",replication);
				printf("--------Test: MULTI SPMU SUBKERNEL DECOUPLED[%dx%d]--------\n", A_ORDER,A_ORDER);
				printf("N of loops:%d\n\n",loop_index);
				printf("Cycle of: Kbcastld() of thread0 is:\t%d Cycles\n", final_perf_ld);
				printf("\n");
				for (int i=0; i<3; i++) {
				...
				matrix_check((int*)output_compare0,(int*)output_compare_s0,A_ORDER);
			#endif
			#ifdef PRINT_DEBUG
				// ENABLE COUNTING -------------------------------------------------------------------------
				final_perf_mem=0;
