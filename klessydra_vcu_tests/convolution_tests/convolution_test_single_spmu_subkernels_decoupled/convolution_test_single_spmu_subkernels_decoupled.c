...
// #define MATRIX_CHECK 0 //in the de-coupled method, and with single_SPMU, this cannot be done
  ...
  for(int i =0;i<A_ORDER; i++)
  {
	...
	array_boundaries[2] = A_ORDER;
		...
		if (Klessydra_get_coreID() == 0) {
			// //------------------------------------------------------------------------------------------
			#ifdef PRINT_NUM_CYCLES
				printf("--------Test: SINGLE SPMU SUBKERNEL DECOUPLED[%dx%d]--------\n", A_ORDER, A_ORDER);
				printf("N of loops:%d\n\n",loop_index);
				printf("Cycle of: Kkmemld() and ksrav() of thread0 is:\t%d Cycles\n", final_perf_ld);
				printf("\n");
				for (int i=0; i<3; i++) {
