...
#error "Please define a way to initialize a memory block."
    ...
    if (results[0].iterations == 0)
    {
        ...
        while (secs_passed < (secs_ret)1)
        {
            ...
            iterate(&results[0]);
            stop_time();
            secs_passed = time_in_secs(get_ctime());
        }
...
/*----------- KLESSYDRA MOD ON ----------- */
    ...
    ftoa((float)time_in_secs(total_time), float_buf, 8);
    ee_printf("Total time (secs): %s\n", float_buf);

    ftoa((float)(default_num_contexts * results[0].iterations / time_in_secs(total_time)), float_buf,8);
    if (time_in_secs(total_time) > 0)
        ee_printf("Iterations/Sec   : %s\n", float_buf  );
