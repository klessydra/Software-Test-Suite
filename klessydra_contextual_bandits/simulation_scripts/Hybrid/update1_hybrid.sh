...
cat hybrid.txt| tail -n 4 |head -n 1 > matrices.txt														# The row -4 contains matrix info, formatted in a compact way

# Preprocessing instructions: 
sed -ir 's/ ;/},{/g' matrices.txt	 																	# The ; char represent a row change and it's substituted by },{
sed -ir 's/ =/}}};\nlinucb_hybrid_policy_object.b_node={.u_value={{/g' matrices.txt	 																	# The ; char represent a row change and it's substituted by },{
sed -ir 's/#/linucb_hybrid_policy_object.A_node={.u_value={{/g' matrices.txt	 																	# The ; char represent a row change and it's substituted by },{
...
sed -iz 's/ _/}}};\nlinucb_hybrid_policy_object.linucb_arms[FFF].b={.u_value={{/g' matrices.txt			# The - char is sobstituted by ending the A definition and starting the assign instruction for b
sed -iz 's/ @/}}};\nlinucb_hybrid_policy_object.linucb_arms[GGG].theta={.u_value={{/g' matrices.txt	# The @ char is substituted by ending the b definition and starting the assign instruction for theta
